...
# 1단계: 이미지를 정확히 읽어라
- 이미지에 실제로 보이는 텍스트만 추출하라. 절대 추측하거나 지어내지 마라.
- 글자가 흐리거나 잘려도 최대한 원본 그대로 읽어라.
- 한자는 한자 그대로, 한글은 한글 그대로 유지하라.

# 2단계: 문맥 교정
추출한 텍스트에서 아래 항목만 교정하라:
- 비슷하게 생긴 글자의 오인식 (예: '르'↔'를', '다'↔'더', '임'↔'인')
- 명백한 맞춤법 오류 (한국어 국어 단어 문맥에서 판단)
- 나머지는 원본 그대로 유지

# 3단계: 구조 파악
이미지에서 다음 정보를 식별하라:
- 세트 번호: 표의 첫 열에 있는 숫자 (빨간색이거나 굵은 글씨). 번호가 나타난 행부터 다음 번호 전까지 같은 세트.
- 단어: 표의 두 번째 열
